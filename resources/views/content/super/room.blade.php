        ...
        <main class="p-6"> 
            ...
            <div class="bg-white rounded-xl shadow-sm mb-8">
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <h2 class="text-xl font-bold text-gray-800">Rooms Management</h2>
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="text-gray-700">
                            ...
                            <tr class="border-t border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="p-4">{{ $b->nama_ruang }}</td>
                                <td class="p-4">{{ $b->lokasi }}</td>
                                <td class="p-4">{{ $b->kapasitas }}</td>
                                <td class="p-4">
                                    <div class="h-32 w-48 overflow-hidden rounded-lg shadow-sm">
                ...
                <div class="p-4 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        ...
                        <a href="{{ $rooms->previousPageUrl() }}" 
                            class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100 {{ $rooms->onFirstPage() ? 'pointer-events-none opacity-50' : '' }}">
                        ...
                        @if ($start > 1)
                            <a href="{{ $rooms->url(1) }}" class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100 {{ $current == 1 ? 'border-blue-500 bg-blue-500 text-white' : '' }}">1</a>
                            @if ($start > 2)
                        ...
                        @if ($end < $last)
                            @if ($end < $last - 1)
                                <span class="px-3 py-2">...</span>
                            @endif
                            <a href="{{ $rooms->url($last) }}" class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100 {{ $current == $last ? 'border-blue-500 bg-blue-500 text-white' : '' }}">{{ $last }}</a>
                        @endif
                    
                        {{-- Tombol selanjutnya --}}
                        <a href="{{ $rooms->nextPageUrl() }}" 
                            class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100 {{ !$rooms->hasMorePages() ? 'pointer-events-none opacity-50' : '' }}">
